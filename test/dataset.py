...
def collate_fn(batch):
    ...
    data['K1s'], data['K2s'], data['Rs'], \
        ...
        data['cx1s'], data['cy1s'], data['f1s'], data['cx2s'], data['cy2s'], data['f2s'],\
             = [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
    for sample in batch:
        ...
        data['cx1s'].append(sample['cx1'])
        data['cy1s'].append(sample['cy1'])
        data['f1s'].append(sample['f1'])
        data['cx2s'].append(sample['cx2'])
        data['cy2s'].append(sample['cy2'])
        data['f2s'].append(sample['f2'])
    ...
    for key in ['K1s', 'K2s', 'Rs', 'ts', 'xs', 'ys', 'T1s', 'T2s','virtPts', 'cx1s', 'cy1s', 'f1s', 'cx2s', 'cy2s', 'f2s']:
...
class CorrespondencesDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        'cx1': cx1, 'cy1':cy1, 'f1':f1, 'cx2':cx2, 'cy2':cy2, 'f2': f2}
